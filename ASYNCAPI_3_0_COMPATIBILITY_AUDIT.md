...
| Operation `reply.messages` | Optional | Partially supported | Repeatable `[ReplyMessage]` attributes author multiple success/error reply messages with independent message and payload schema ids; the single-reply `ReplyMessagePayloadType` surface remains supported, including `ReplyMessagePayloadSchemaId`; reply refs are validated against their channel and obviously identical payload/header/content-type/bindings alternatives are rejected, but Saunter does not prove the spec's "one and only one" invariant for all overlapping schemas |
...
- Reply metadata authoring is supported end-to-end for `reply.channel`, `reply.address`, and `reply.messages`, including multiple typed reply variants, explicit reply payload schema ids, and generated reply channels from attribute metadata. Dynamically addressed reply channels intentionally omit request-channel bindings because the runtime expression determines the physical destination.
...
- Reply message references are checked against the generated reply channel, and alternatives with identical payload schema, headers schema, content type, and bindings references are rejected. Saunter does not determine whether more subtly overlapping payload/header schemas allow an actual reply to validate against more than one listed Message Object. AsyncAPI 3.0.0 requires every processed reply to be valid against one, and only one, referenced message, so authors must keep reply variants structurally distinguishable.
  - See the AsyncAPI 3.0.0 `Operation Reply Object` `messages` requirement, [AttributeMessageResolver.cs](src/Saunter/AttributeProvider/AttributeMessageResolver.cs), and [AsyncApiDocumentValidator.cs](src/Saunter/AttributeProvider/AsyncApiDocumentValidator.cs).
