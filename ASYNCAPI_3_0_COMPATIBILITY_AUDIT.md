...
- For the supported reusable component maps, the descriptor surface still only exposes concrete objects, not the broader `Object | Reference Object` authoring model the spec allows.
  - See [AsyncApiComponentsDescriptor.cs](src/Saunter/Descriptors/AsyncApiComponentsDescriptor.cs#L9-L28).
- The validator does not enforce the reply-address/channel invariant from AsyncAPI 3.0.0. The spec requires the referenced reply channel to have `address: null` or no address when `reply.address` is present, but the current validator only checks that the reply channel exists.
  - See [AsyncApiDocumentValidator.cs](src/Saunter/AttributeProvider/AsyncApiDocumentValidator.cs#L78-L88) and the AsyncAPI 3.0.0 spec `Operation Reply Object` section.
