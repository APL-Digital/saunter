...
| Operation `reply.messages` | Optional | Supported | Repeatable `[ReplyMessage]` attributes author multiple success/error reply messages with independent message and payload schema ids; the single-reply `ReplyMessagePayloadType` surface remains supported, including `ReplyMessagePayloadSchemaId`; the provider can auto-create a reply channel for message-only replies, dynamic replies (`reply.address`), or `ReplyChannelAddress` |
